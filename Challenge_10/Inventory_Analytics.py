def main():
    ...
    while True:
        ...
        if option == 1:
            ...
            price_list = []
            for item in inventory:
                price_list.append(inventory[item]["price"] * inventory[item]["quantity"])
            max_price = max(price_list)
            most_valuable_items = []
            for item in inventory:
                if inventory[item]['price'] * inventory[item]['quantity'] == max_price:
                    most_valuable_items.append(item)
                else:
                    continue
            if len(most_valuable_items) == 1:
                print(f"Most valuable item: {most_valuable_items[0]} (Total value: {max_price})")
            elif len(most_valuable_items) > 1:
                print(f"Most valuable items: {', '.join(most_valuable_items)} (Individual value: {max_price})")
        elif option == 9:
            price_list = []
            for item in inventory:
                price_list.append(inventory[item]["price"])
...
def get_option():
    while True:
        ...
        print("8. Most valuable item")
        print("9. Average item price")
